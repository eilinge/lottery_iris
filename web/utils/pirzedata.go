...
/*
引入奖品池之前, 验证奖品的库存, 直接使用数据库中该奖品的leftNum来进行验证
奖品有多少库存, 就有多少奖品, 很可能导致在开始的一段时间内, 就将所有奖品进行分发出去了
为了避免不同的时间段发放不均衡, 引入了奖品池, 作为奖品库存的中间缓冲地带, 发奖时, 先检查奖品池
中的剩余数量, 不直接检查数据库库存

奖品池的全量缓存, Hash中的计数器, utils/Prizedata
	GetGiftPoolNum 从奖品池验证奖品库存数量
	prizeServiceGift 从奖品池扣减奖品的库存数量
	PirzeGift 从奖品池发奖
*/

/*
发奖计划, 让奖品的数量合理的分配到发奖周期内
每天不同时段根据访问量来设置概率更加合理
根据发奖计划填充奖品池, 保证奖品可以有节奏的发放出去
	操作redis缓存时, 需要考虑到并发问题, 设置二次补发, 有了奖品池的缓冲地带, 保证奖品可以有节奏的发放出去
*/

// ResetGiftPrizeData 重置一个奖品的发奖周期信息
// 奖品剩余数量也会重新设置为当前奖品数量
// 奖品的奖品池有效数量则会设置为空
...
func DistributionGiftPool() int {
	...
	if list != nil && len(list) > 0 {
		for _, gift := range list {
			...
			if err != nil {
				log.Println("prizedata.DistributionGiftPool Unmarshal err= ", err)
			} else {
				index := 0
				giftNum := 0
				for i, data := range cronData {
					...
					num := data[1] // 数量
					if ct <= now {
						giftNum += num // 获取有效的发奖计划, 放入奖品池的总数量
						index = i + 1 // 已更新的总数量
					} else {
						break
					}
				...
				// 跟新奖品的发奖计划
				if index > 0 {
					if index >= len(cronData) { // 到达末尾, 将发奖计划变为0
					...
					if err != nil {
						log.Println("prizedata.DistributionGiftPool Marshal", cronData, ", err= ", err)
					}
					...
					if err != nil {
						log.Println("prizedata.DistributionGiftPool update err= ", err)
					}
				}
			}
		}
...
// getGiftPrizeDataOneDay 将给定的奖品数量分布到这一天的时间内
// 结构为： [hour][minute]num
...
// formatGiftPrizeData 将每天、每小时、每分钟的奖品数量，格式化成具体到一个时间（分钟）的奖品数量
...
// incrGiftPool 根据计划数据，往奖品池增加奖品数量
func incrGiftPool(id, num int) int {
	return incrServGiftPool(id, num)
}

// incrServGiftPool 往奖品池增加奖品数量，redis缓存，根据计划数据
...
// prizeServCodeDiff 优惠券发放，使用redis的方式发放
...
// setGiftPool 设置奖品池的数量
func setGiftPool(id, num int) {
	setServGiftPool(id, num)
}

// setServGiftPool 设置奖品池的数量，redis缓存
...
// getServGiftPoolNum 获取当前奖品池中的奖品数量，从redis中
